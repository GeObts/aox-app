    ...
    async syncInternal(workflowId, config) {
        ...
        const [currentTriggers, currentTasks, currentWorkspace] = await Promise.all([
            ...
            this.client
                ...
                .catch(() => []),
            this.client.get(`/workspaces/${workflowId}`).catch(() => null),
        ]);
        ...
        if (config.tasks && currentWorkspace) {
            const currentAgentIds = new Set((currentWorkspace.agents || []).map((a) => a.id));
            const requiredAgentIds = new Set(config.tasks
                .map((t) => typeof t.agentId === "string" ? parseInt(t.agentId) : t.agentId)
        ...
        if (config.triggers) {
            const triggersWithConnections = await Promise.all(config.triggers.map(async (t) => {
                ...
                const integrationConnectionId = await this.client.integrations.getOrCreateConnection(identifier);
        ...
        if (config.edges || config.triggers || config.tasks) {
            ...
            if (config.triggers) {
                for (const t of config.triggers) {
                    ...
                    const tId = t.id;
                    const triggerId = tId || triggerNameToId.get(triggerName) || `new-${triggerName}`;
                    nodes.push({
            ...
            if (config.tasks) {
                for (const t of config.tasks) {
                    ...
                    const outputPorts = Object.entries(outputOptions).map(([key, opt]) => ({
                        id: key,
                        name: opt.name || key,
                    }));
                    nodes.push({
